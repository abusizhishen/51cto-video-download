...
class cto(object):
    ...
    def get_download_url(self,lesson_id,video_id,dp = 'high'):
        #dp = 'general'
        url = 'http://edu.51cto.com//center/player/play/m3u8?lesson_id=%s&id=%d&dp=%s&type=wejoboutcourse&lesson_type=course'%(lesson_id,video_id,dp)
    ...
    def get_course_info(self, headers, train_id, train_course_id, page):
        infos = []
        for i in range(1,page+1):
            url = 'http://edu.51cto.com/center/wejob/usr/course-infoajax?train_id=%d&train_course_id=%d&page=%d&size=20'%(train_id, train_course_id ,i)
    ...
    def get_train_courses(self,headers,train_id):
        url = 'http://edu.51cto.com/center/wejob/usr/courseajax?train_id=%d&page=1&size=20'%(train_id)
...
#77 164
train_id = 94
path = 'd:/videos/%d'%(train_id)
path = 'd:/Python高级自动化开发工程师微职位/'#%d'%(train_id)
#path = 'd:/深度学习'
obj.train(headers, train_id, path)
# res = obj.get_train_courses(headers, train_id)
# #res = obj.get_course_info(headers, train_id,536,2)
# for i in res:
#     print(i)
